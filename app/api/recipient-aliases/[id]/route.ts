...
export async function handleUpdateRecipientAliasRequest(
  request: Request,
  options?: { accountId?: string | null; aliasId?: string | null }
) {
  const accountId = options?.accountId ?? "";
  const aliasId = options?.aliasId?.trim() ?? "";
...
export async function handleDeleteRecipientAliasRequest(
  request: Request,
  options?: { accountId?: string | null; aliasId?: string | null }
) {
  const accountId = options?.accountId ?? "";
